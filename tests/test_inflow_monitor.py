...
class FetchTransactionsTests(unittest.TestCase):
    ...
    def test_whale_alert_falls_back_to_ripple_data_when_empty(self):
        ...
        with mock.patch(
            ...
            txs, outflows = monitor.fetch_transactions()
    ...
    def test_build_flows_prefers_cached_when_ripple_data_empty(self):
        ...
        ) as mock_cached, mock.patch(
            "xrpl_inflow_monitor.fetch_cached_outflows", return_value=[]
        ) as mock_out_cached:
            flows, outflows = monitor.build_flows()

        self.assertEqual(flows, [{"xrp": 4}])
        self.assertEqual(outflows, [])
        mock_ripple.assert_called_once()
        mock_cached.assert_called_once()
        mock_out_cached.assert_called_once()
    ...
    def test_sample_flows_provide_structured_payload(self):
        inflows, outflows = monitor.sample_flows()

        self.assertGreaterEqual(len(inflows), 2)
        self.assertEqual(outflows, [])
        for flow in inflows:
...
class RippleData403HandlingTests(unittest.TestCase):
    ...
    def test_403_blocks_address_without_global_cooldown(self):
        ...
        with mock.patch(
            ...
            flows, outflows = monitor.fetch_transactions_ripple_data()

        self.assertIn("bad", monitor.ripple_data_blocked_addresses)
        self.assertEqual(monitor.ripple_data_cooldown_until, 0)
        self.assertEqual(len(flows), 1)
        self.assertEqual(len(outflows), 0)
        self.assertEqual(flows[0].get("to_address"), "good")
        mock_sleep.assert_called()

    def test_non_403_failures_trigger_backoff_cooldown(self):
        monitor.ripple_data_blocked_addresses.clear()
        monitor.ripple_data_failure_streak = 0
        monitor.ripple_data_cooldown_until = 0

        with mock.patch(
            "xrpl_inflow_monitor.monitored_addresses", return_value=["bad"]
        ), mock.patch("xrpl_inflow_monitor.requests.get") as mock_get, mock.patch(
            "xrpl_inflow_monitor.time.sleep"
        ) as mock_sleep, mock.patch("xrpl_inflow_monitor.time.time", return_value=1000):
            mock_get.return_value = mock.Mock(status_code=500, ok=False)

            for _ in range(3):
                monitor.fetch_transactions_ripple_data()

        self.assertGreater(monitor.ripple_data_cooldown_until, 1000)
        self.assertGreaterEqual(monitor.ripple_data_failure_streak, 3)
        mock_sleep.assert_called()
