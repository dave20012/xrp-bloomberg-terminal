# exchange_addresses.py
# Curated list of major XRPL exchange hot wallets (non-exhaustive).
# Used for tagging XRPL → exchange inflows and weighting flows.

EXCHANGE_ADDRESSES = {
    # Global majors
    "Binance": [
        "rEb8TK3gBgk5auZkwc6sHnwrGVJH8DuaLh",
        "rEy8TFcrAPvhpKrwyrscNYyqBGUkE9hKaJ",
        "rs8ZPbYqgecRcDzQpJYAMhSxSi5htsjnza",
        "rH71Wv8YCeHCbMfWDrFGCDuUBhxQB2HLN1",
        "rJb5KsHsDHF1YS5B5DU6QCkH5NsPaKQTcy",
        "rHxPQVkTvqbdq1pXbgVGH2PTQdBKkYqKD",
        "radCSacS59EEzcJ7sYCTTpNRYnD7UvGAYc",
        "rhQADfs6UxfP7iUPwsU7b3uwDVQLLgFcu8",
        "rJpj1Mv21gJzsbsVnkp1U4nqchZbmZ9pM5",
    ],
    "Kraken": [
        "rLHzPsX6oXkzU2qL12kHCH8G8cnZv1rBJh",
    ],
    "Bitstamp": [
        "rDsbeomae4FXwgQTJp9Rs64Qg9vDiTCdBv",
    ],
    "Bitfinex": [
        "rDkxe2eY5dL5sXK4GzF2bPZVuyz5v5Wg6r",
    ],
    "Bittrex": [
        "rE3sV9KSWeSiaAapsZGcSDfiFRSshK8Bqj",
        "rPVMhWBsfF9iMXYj3aAzJVkPDTFNSyWdKy",
    ],

    # US / EU retail / custodial
    "Uphold": [
        "rhub8VRN55s94qWKDv6jmDy1pUykJzF3wq",  # common public one
        "rQrQMKhcw3WnptGeWiYSwX5Tz3otyJqPnq",
        "ravfDRniJw2vydfJ9EAhLGpRfwXibSLE7R",
        "rKfzfrk1RsUxWmHimWyNwk8AoWHoFneu4m",
        "r2gaSDKcpDoC8b6emtZPzuPVs52ZGT1MA",
        "rMdG3ju8pgyVh29ELPWaDuA74CpWW6Fxns",
        "r9sc3QxNuex2EMQfPLPXPVhVuEk6mD8K8P",
    ],
    "GateIO": [
        "rGtB2QQzBBRUPXbfqiWWZ7d9m3F2ZG8ZRt",
    ],
    "KuCoin": [
        "rKiCet8SdvWxPXnAgYarFUXMh1zCPz432Y",
        "rBxszqhQkhPALtkSpGuVeqR6hNtZ8xTH3T",
        "rEW8BjpMyFZfGMjqbykbhpnr4KEb2qr6PC",
        "rNFugeoj3ZN8Wv6xhuLegUBBPXKCyWLRkB",
        "rwYViw74UxphPuaCvozsGi99k5KXce51yC",
        "rhKrvXY8zRvVyDLT4nfXB86HyhqQyvcr4c",
        "rUW9toSjQkLY6EspdnBJP2paG4hWKmNbMh",
        "rBWFDgD785LUBDDgcCi9dAW6TH2Gmtq8CM",
        "r4pxv9txfQTyVkQUetNmyJqDy7s5fPPVfB",
    ],
    "Bybit": [
        "rUrujDEigaJqjxLzzXRjUWZ2DweN34JXqY",
        "rMrgNBrkE6FdCjWih5VAWkGMrmerrWpiZt",
        "rNFKfGBzMspdKfaZdpnEyhkFyw7C1mtQ8x",
        "r3WiKULZw4ySPG7ixU4WdW2TaUFwS9Q1pz",
        "rJBSXZxjkuExQPKWtZYpentW2PnCjZGUoL",
        "rJn2zAPdFA193sixJwuFixRkYDUtx3apQh",
    ],

    # Japan / Korea (requested)
    "Bitbank": [
        "r97KeayHuEsDwyU1yPBVtMLLoQr79QcRFe",
        "rw7m3CtVHwGSdhFjV4MyJozmZJv3DYQnsA",
        "rwggnsfxvCmDb3YP9Hs1TaGvrPR7ngrn7Z",
        "rLbKbPyuvs4wc1h13BEPHgbFGsRXMeFGL6",
    ],
    "Bithumb": [
        "rLdfNmMmaAZS96DH4hAXTz3UvWs4L2JFLM",
        "r9LHiNDZvpLoWPoKnbH2JWjFET8zoYT4Y5",
        "r4fPftbRJK92DmhGtu2kdGL6QzZaM6o1F6",
        "rnArodsZcfbJoJ9WJ4fZEmBsjNX1889Kd7",
        "rLdFvFyH1BpPargdaUn41srkyNcCgFKKX4",
        "r9hUMZBc3MWRc4YdsdZgNCW5Qef8wNSXpb",
    ],
    "Upbit": [
        "rNzT5xopUaJK8L7mHbThUGKaHoct62dNBC",
        "rEVrb2XPMxs3Wq1zKgFaYZers9nKCWUNd5",
        "rQBfg7A7QtTyXR2FtzX19WmkfiDKkMV8ge",
        "rBszWJzYpNoqoY4xKuGUpN23b6EBT41ocF",
        "rN9qNpgnBaZwqCg8CvUZRPqCcPPY7wfWep",
        "rpHQoK8FWoBjiGEAK9UwCxxdcboXETTpkY",
        "rEahGW5VLYQH2vGBfFmbv9BBpWSTDoukGR",
        "rEan25j81J2WqkMeXR8iqtLv93KJvmtfCB",
        "rJcYzMVVtHaLhT9E69J4xq9j5TqFosyNS3",
    ],

    # LatAm / others of note
    "Bitso": [
        "rGfGdVYLDSbji5mqfMvdpx4c8JyyfqVFgf",
        "rwApWND4wwXXiYRBvTdrW981YssreQcRdw",
        "raXLsnnJVaLMDixEoXHXe56WQXKczbD8ub",
        "rG6FZ31hDHN1K5Dkbma3PSB5uVCuVVRzfn",
        "rLSn6Z3T8uCxbcd1oxwfGQN1Fdn5CyGujK",
        "rfEu1Wnr7LxStoFx8DBdzgr8M16FBUbH3K",
        "rHZaDC6tsGN2JWGeXhjKL6664RNCq5hu4B",
    ],
    "Bitrue": [
        "raLPjTYeGezfdb6crXZzcC8RkLBEwbBHJ5",
        "rfKsmLP6sTfVGDvga6rW6XbmSFUzc3G9f3",
        "rNYW2bie6KwUSYhhtcnXWzRy5nLCa1UNCn",
        "r4DbbWjsZQ2hCcxmjncr7MRjpXTBPckGa9",
        "rKq7xLeTaDFCg9cdy9MmgxpPWS8EZf2fNq",
    ],
    "Bitkub": [
        "rDnLSfDv8nG7D7P59QXi78XdnRJ6GHroFJ",
        "rE3Cc3i6163Qzo7oc6avFQAxQE4gyCWhGP",
        "rpXTzCuXtjiPDFysxq8uNmtZBe9Xo97JbW",
        "r3ySY1mmrvfSNvrTdQVveTH9y8LbjisVv4",
    ],

    # You can append more exchanges here as needed
}

# Exchange impact weights: how much a unit of inflow should count in scoring
EXCHANGE_WEIGHTS = {
    "Binance": 1.00,
    "Coinbase": 0.95,
    "Kraken": 0.85,
    "Bitstamp": 0.80,
    "Uphold": 0.70,
    "KuCoin": 0.65,
    "GateIO": 0.60,
    "bitbank": 0.60,
    "bitFlyer": 0.60,
    "Coincheck": 0.60,
    "Upbit": 0.75,
    "Bithumb": 0.75,
}


# Ripple corporate / treasury style wallets (for OTC→exchange tagging)
RIPPLE_CORP_ADDRESSES = [
    # Commonly referenced Ripple / escrow / distribution wallets.
    # This list is intentionally minimal; extend if you have a vetted list.
    "rB3WNZc45gxzW31zxfXdkx8HusAhoqscPn",
    "r9UUEXn3cx2seufBkDa8F86usfjWM6HiYp",
    "rDdXiA3M4mYTQ4cFpWkVXfc2UaAXCFWeCK",
    "rMhkqz3DeU7GUUJKGZofusbrTwZe6bDyb1",
    "r9NpyVfLfUG8hatuCCHKzosyDtKnBdsEN3",
    "rKDvgGUsNPZxsgmoemfrgXPS2Not4co2op",
    "rKwJaGmB5Hz24Qs2iyCaTdUuL1WsEXUWy5",
    "rN8pqRwLYuuvY7pUHurybPC8P6rLqVsu6o",
    "rMQ98K56yXJbDGv49ZSmW51sLn94Xe1mu1",
    "rGKHDyj4L6pc7DzRB6LWCR4YfZfzXj2Bdh",
    "rHGfmgv54kpc3QCZGRXEQKUhLPndbasbQr",
    "rKveEyR1SrkWbJX214xcfH43ZsoGMb3PEv",
    "rw2hzLZgiQ9q62KCuaTWuFHWfiX7JWg3wY",
    "rsjFB8mPWqiZgPUaVh8XYqdfa59PE2d5LG",
    "rDqGA2GfveHypDguQ1KXrJzYymFZmKxEsF",
    "rp6aTJmW3nq1aKt3Jmuz4DPRxksT5PBjpH",
    "rpynXLU7rAHtdKmdwgiVqLSfz9xwDvK4hy",
    "rBZz5VCEXpYzvhN8jWCC2Sb2QVDzsM5WKs",
    "rhtufNsYfrozs4GvSq4HMYcR9y3dg8FWdC",
    "rBg2FuZT91C52Nny68houguJ4vt5x1o91m",
    "rJqiMb94hyz41SBTNr2AyPNW8AzELa8nE",
    "rhyp1uMC9Xyj3Px8amUH3xVv6tbjYJkojs",
    "r3F5NsyiGwVYs2Rs3cCcrVw4t5wZP2ZxRr",
    "r4MoybfgCHDoUByYyMejimaX6a8CEtWtav",
    "rhbWRsNE4v4neB5KvscrwCvcETv7c1vDJf",

    
]
